...
async function runDemo(port) {
	...
	try {
		...
		await fetch(`${API}/plants`, {
			...
			body: JSON.stringify({ id: "A", name: "Шаг1", nextCareDate: "2026-05-25", complexity: 1, healthIndex: 80 }),
		});
		await fetch(`${API}/plants`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ id: "B", name: "Шаг2", nextCareDate: "2026-05-25", complexity: 1, healthIndex: 80 }),
		});
		await fetch(`${API}/plants`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ id: "C", name: "Шаг3", nextCareDate: "2026-05-25", complexity: 1, healthIndex: 80 }),
		});
		...
		await fetch(`${API}/relations`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ id1: "A", id2: "B", type: "care_sequence", weight: 1 }),
		});
		await fetch(`${API}/relations`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ id1: "B", id2: "C", type: "care_sequence", weight: 1 }),
		});
		...
		await fetch(`${API}/relations`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ id1: "A", id2: "C", type: "conflict", weight: -1 }),
		});
		...
		console.log(
			"   2. В plant-manager.js НЕТ пробелов в ключевых словах (this.compatGraph, const, =>, getNeighbors)"
		);
	}
}
